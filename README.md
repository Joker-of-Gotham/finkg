...
关系类型在 Neo4j 里**直接是中文动作**，并强制携带机制与量化属性：

```cypher
(:Company {caption:"赣锋锂业"})
  -[:`向客户长协供应电池级碳酸锂` {
      机制: "长协锁量、价格随基准指数联动，锂价波动经采购成本进入宁德时代营业成本",
      语义层: "supply_operation", 合同类型: "长期协议",
      计价方式: "指数联动", 锁量: "不低于8万吨/年", 占采购比: "约15%"}]->
(:Company {caption:"宁德时代"})
```

写「相关」「影响」「关联」的边会被判为不可分析。写不出机制，说明这一跳还没研究清楚。

### 4. 十跳要是机制，不是长度

只有业务实体之间的边算跳（`Document`/`Observation`/`Metric` 不算，否则「公司→文档→观测→指标→公司」能把 0 跳灌成 4 跳）。一条长路径要算真纵深，必须同时满足四条：

| 判据 | 挡住的是 |
| --- | --- |
| 跨 ≥3 个语义层 | 同层展开冒充机制传导 |
| 关系类型数 ≥ ⌈跳数/2⌉ | 「A供应B供应C供应D」单一关系接龙 |
| 每跳有事实支撑且写了机制 | 中间靠猜连起来 |
| 连续推断跳 ≤2 | 整条链悬空 |
| 至少 2 条边不重叠的独立见证 | 一根主干换尾巴充当多条路径 |
...
**质量只判内容，不判格式。** 不检查 JSON 有没有某个 key、不算工件哈希、不把节点数量本身当目标。判的是：这个数字有没有单位币种期间口径、这条引文能不能在原文里找到、这个节点够不够支撑判断、这条边说不说得清机制、这条路径是不是真跨了机制层、检索回来的数据有多少真的进了图。格式对但内容空的图一定不通过；格式随意但内容扎实的图会通过。
