...
(:Company {caption:"赣锋锂业"})
  -[:`向客户长协供应电池级碳酸锂` {
      id: "R00004", relation: "向客户长协供应电池级碳酸锂",
...
好处是 Browser 里一眼读懂，Cypher 里也能按中文关系名直接查。代价是关系类型很多——用 `type(r)` 而不是写死类型名来做通用查询。
...
fg neo4j load --force       # 质量报告 ok=false 也装
```
...
所以装载要么完整成功，要么库完全没动。装载前会看 `reports/quality.json`；`ok=false` 时拒绝，除非 `--force`。
