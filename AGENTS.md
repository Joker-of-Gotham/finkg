...
- **关系是具体中文金融动作 + 一句话机制 + 量化属性。** 写「相关/影响/关联」的边判为不可分析。
- **纵深服务于机制问题。** 只有业务实体之间的边算跳；跨层不足、同一关系重复接龙、某跳无证据的长链会被判为非实质。
- **人在回路用结构化提问在对话里问**（摆当前判断、证据样本、2~3 个真实取舍、建议、为何现在必须定），答复用 `fg align` / `fg answer` 记进明文台账。
- **没有 Gate、签名、令牌、哈希链、密钥。** `fg quality` 的 `ok=false` 只意味着「还不能对外声称达标」，它不批准也不阻止任何人类决定；要装中间快照用 `fg neo4j load --force`。
